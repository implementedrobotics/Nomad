...
#  endif
    ...
    _interpreter() {
	    ...
	    PyObject* stylename = PyString_FromString("matplotlib.style");
        if (!pyplotname || !pylabname || !matplotlibname || !cmname || !stylename) {
        ...
        //PyObject_CallMethod(pyplotname, const_cast<char*>("style.use"), const_cast<char*>("ggplot"), s_backend.c_str());
        ...
        PyObject* matplotlib_style = PyImport_Import(stylename);
        Py_DECREF(stylename);
        if (!matplotlib_style) { throw std::runtime_error("Error loading module style!"); }


	    s_python_function_style = safe_import(matplotlib_style, "use");
...
inline void style(const std::string &str, const std::map<std::string, std::string> &keywords = {})
{
    PyObject* pystr = PyString_FromString(str.c_str());
    PyObject* args = PyTuple_New(1);
    PyTuple_SetItem(args, 0, pystr);
    ...
    PyObject* res = PyObject_Call(detail::_interpreter::get().s_python_function_style, args, kwargs);
    if(!res) throw std::runtime_error("Call to style.use() failed.");
